...
<div class="stok-container">
    <div class="card">
        <div class="card-header">
            <h2 class="card-title">Daftar Stok Opname</h2>
            <a href="{{ route('stok-opname.create') }}" class="btn btn-primary">
                <i class="fas fa-plus"></i> Tambah Opname
            </a>
        </div>
        <div class="card-body">
            <form method="GET" action="{{ route('stok-opname.index') }}" class="search-filter-container">
                ...
                <div class="filter-group">
                    ...
                    <input type="date" 
                           class="date-input" 
                           name="tanggal_opname" 
                           value="{{ request('tanggal_opname') }}"
                           id="dateFilter">
                    <button type="submit" class="btn btn-primary">
                        <i class="fas fa-filter"></i> Filter
                    </button>
                    @if(request()->has('search') || request()->has('status') || request()->has('tanggal_opname'))
                    ...
                    <tbody>
                        @forelse ($opnames as $index => $opname)
                            <tr>
                                ...
                                <td>{{ $opname->rak ?? '-' }}</td>
                                <td class="text-right">{{ number_format($opname->stok_sistem) }}</td>
                                <td class="text-right">{{ number_format($opname->stok_fisik) }}</td>
                                ...
                                <td class="action-cell">
                                    <a href="{{ route('stok-opname.show', $opname->id) }}" class="btn btn-outline btn-sm" title="Detail">
                            ...
                            <tr>
                                <td colspan="10">
                                    <div class="empty-state">
                                        ...
                                        <h4 class="empty-text">Belum ada data stok opname</h4>
                                        <a href="{{ route('stok-opname.create') }}" class="btn btn-primary">
                                            <i class="fas fa-plus"></i> Tambah Opname
                                        </a>
                                    </div>
...
<script>
    ...
    document.getElementById('searchInput').addEventListener('keyup', function() {
        ...
        rows.forEach(row => {
            ...
            const warehouseName = row.cells[3].textContent.toLowerCase();
            if (productName.includes(searchValue) || warehouseName.includes(searchValue)) {
                row.style.display = '';
            } else {
