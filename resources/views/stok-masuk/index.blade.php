...
<div class="stok-container">
    <div class="card">
        <div class="card-header">
            <h2 class="card-title">Daftar Stok Masuk</h2>
            <a href="{{ route('stok-masuk.create') }}" class="btn btn-primary">
                <i class="fas fa-plus"></i> Tambah Stok Masuk
            </a>
        </div>
        <div class="card-body">
            <form method="GET" action="{{ route('stok-masuk.index') }}" class="search-filter-container">
                ...
                <div class="filter-group">
                    ...
                    @if(request()->has('search') || request()->has('status') || request()->has('tanggal_masuk'))
                        ...
                        @forelse($stokMasuk as $index => $item)
                            <tr>
                                ...
                                <td class="action-cell">
                                    <a href="{{ route('stok-masuk.show', $item->id) }}" class="btn btn-outline btn-sm" title="Detail">
                                        <i class="fas fa-eye"></i> Detail
                                    </a>
                                    
                                    @if($item->status == 'pending' && auth()->user()->can('approve-stok-masuk'))
                                        <form action="{{ route('stok-masuk.approve', $item->id) }}" method="POST" style="display:inline;">
                                            @csrf
                                            @method('PUT')
                                        ...
                                        <form action="{{ route('stok-masuk.reject', $item->id) }}" method="POST" style="display:inline;">
                                            @csrf
                                            @method('PUT')
                            ...
                            <tr>
                                <td colspan="9">
                                    <div class="empty-state">
                                        ...
                                        <h4 class="empty-text">Belum ada data stok masuk</h4>
                                        <a href="{{ route('stok-masuk.create') }}" class="btn btn-primary">
                                            <i class="fas fa-plus"></i> Tambah Stok Masuk
                                        </a>
                                    </div>
